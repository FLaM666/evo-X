...
void Spell::EffectScriptEffect(uint32 effIndex)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 65917:
                {
                    ...
                    {
                        if (skillval >= 150)
                            unitTarget->CastSpell(unitTarget, 66123, true);
                        else
                            unitTarget->RemoveSpellsCausingAura(SPELL_AURA_MOUNTED);
                    }
                    return;
        ...
        // Magic Rooster Mount
              ...
              unitTarget->RemoveSpellsCausingAura(SPELL_AURA_MOUNTED); // its not right but i dont now how to handle it otherway
                    }
                    return;
                }
