...
struct MANGOS_DLL_DECL boss_razorscaleAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho)
    {
        ...
        m_creature->SendMonsterMove(RazorscaleBossX[1], RazorscaleBossY[1], RazorscaleBossZ[1], 0, m_creature->GetMonsterMoveFlags(), 1);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Timetoground < diff && airphase)
        {
            ...
            m_creature->SendMonsterMove(RazorscaleBossX[2], RazorscaleBossY[2], RazorscaleBossZ[2], 1.564225, m_creature->GetMonsterMoveFlags(), 1);
            grounded = true;
            Stun_Timer = 2000;
            Ground_Cast = 40000;
