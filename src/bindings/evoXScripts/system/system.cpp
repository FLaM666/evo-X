...
void SystemMgr::LoadVersion()
{
    ...
    QueryResult* pResult = SD2Database.PQuery("SELECT version FROM evoXScripts_version LIMIT 1");
    ...
    {
        error_log("evo-X: Missing `evo-X_db_version` information.");
        outstring_log("");
    }
}
...
{
    outstring_log("evo-X: Loading Custom Texts...");
    ...
    outstring_log("evo-X: Loading Custom Texts additional data...");
    ...
    if (pResult)
    {
        ...
        do
        {
            ...
            if (iId >= 0)
            {
                error_db_log("evo-X: Entry %i in table `custom_texts` is not a negative value.", iId);
                continue;
            }

            if (iId > TEXT_SOURCE_RANGE*2 || iId <= TEXT_SOURCE_RANGE*3)
            {
                error_db_log("evo-X: Entry %i in table `custom_texts` is out of accepted entry range for table.", iId);
                continue;
            }
            ...
            {
                if (!GetSoundEntriesStore()->LookupEntry(pTemp.uiSoundId))
                    error_db_log("evo-X: Entry %i in table `custom_texts` has soundId %u but sound does not exist.", iId, pTemp.uiSoundId);
            }

            if (!GetLanguageDescByID(pTemp.uiLanguage))
                error_db_log("evo-X: Entry %i in table `custom_texts` using Language %u but Language does not exist.", iId, pTemp.uiLanguage);

            if (pTemp.uiType > CHAT_TYPE_ZONE_YELL)
                error_db_log("evo-X: Entry %i in table `custom_texts` has Type %u but this Chat Type does not exist.", iId, pTemp.uiType);
...
void SystemMgr::LoadScriptWaypoints()
{
    ...
    outstring_log("evo-X: Loading Script Waypoints for %u creature(s)...", uiCreatureCount);
    ...
    if (pResult)
    {
        ...
        do
        {
            ...
            if (!pCInfo)
            {
                error_db_log("evo-X: DB table script_waypoint has waypoint for non-existant creature entry %u", pTemp.uiCreatureEntry);
                continue;
            }

            if (!pCInfo->ScriptID)
                error_db_log("evo-X: DB table script_waypoint has waypoint for creature entry %u, but creature does not have ScriptName defined and then useless.", pTemp.uiCreatureEntry);
