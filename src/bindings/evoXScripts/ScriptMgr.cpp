...
void LoadDatabase()
{
    std::string strSD2DBinfo = SD2Config.GetStringDefault("evoXScriptsDatabaseInfo", "");

    if (strSD2DBinfo.empty())
    {
        error_log("evo-X: Missing evo-X database info from configuration file. Load database aborted.");
        return;
    }
    ...
    {
        outstring_log("evo-X: evoXScripts database at %s initialized.", strSD2DBinfo.c_str());
        outstring_log("");
    ...
    {
        error_log("evo-X: Unable to connect to Database. Load database aborted.");
        return;
    }
...
void DoScriptText(int32 iTextEntry, WorldObject* pSource, Unit* pTarget)
{
    if (!pSource)
    {
        error_log("evo-X: DoScriptText entry %i, invalid Source pointer.", iTextEntry);
        return;
    }

    if (iTextEntry >= 0)
    {
        error_log("evo-X: DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.",
    ...
    if (!pData)
    {
        error_log("evo-X: DoScriptText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.",
            pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);

        return;
    }

    debug_log("evo-X: DoScriptText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u",
    ...
    if (pData->uiSoundId)
    {
        if (GetSoundEntriesStore()->LookupEntry(pData->uiSoundId))
            ...
            error_log("evo-X: DoScriptText entry %i tried to process invalid sound id %u.", iTextEntry, pData->uiSoundId);
    }
    ...
    if (pData->uiEmote)
    {
        if (pSource->GetTypeId() == TYPEID_UNIT || pSource->GetTypeId() == TYPEID_PLAYER)
            ...
            error_log("evo-X: DoScriptText entry %i tried to process emote for invalid TypeId (%u).", iTextEntry, pSource->GetTypeId());
    }
    ...
    switch(pData->uiType)
    {
        ...
        case CHAT_TYPE_WHISPER:
        {
            if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER)
                ...
                error_log("evo-X: DoScriptText entry %i cannot whisper without target unit (TYPEID_PLAYER).", iTextEntry);
...
void Script::RegisterSelf()
{
    ...
    {
        debug_log("evo-X: RegisterSelf, but script named %s does not have ScriptName assigned in database.",(this)->Name.c_str());
        delete this;
    }
}
...
{
    debug_log("evo-X: Gossip selection, sender: %u, action: %u", uiSender, uiAction);
...
{
    debug_log("evo-X: Gossip selection with code, sender: %u, action: %u", uiSender, uiAction);
